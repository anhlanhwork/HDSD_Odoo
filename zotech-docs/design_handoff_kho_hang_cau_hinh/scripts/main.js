...
(function () {
  ...
  function initSidebarTree() {
    ...
    $all(".sb-group").forEach(function (row) {
      ...
      row.__setOpen = setOpen;
      if (row.classList.contains("sb-subgroup")) {
        subGroups.push({ row: row, node: node, children: children, setOpen: setOpen });
      }
    });

    // Single delegated click on sidebar handles ALL group toggles
    var sidebarEl = $(".sidebar");
    if (sidebarEl) {
      sidebarEl.addEventListener("click", function (e) {
        // Ignore clicks that originate from a leaf link (sb-sub / sb-leaf)
        if (e.target.closest(".sb-sub, .sb-leaf")) return;
        var row = e.target.closest(".sb-group");
        if (!row) return;
        if (row.__setOpen) {
          if (row.tagName === "A") e.preventDefault();
          row.__setOpen(row.getAttribute("aria-expanded") !== "true");
        }
      });
    }

    $all(".sb-sub, .sb-leaf").forEach(function (leaf) {
      leaf.addEventListener("click", function () { /* sidebar state preserved */ });
    });
  ...
  function initFeaturePanels() {
    ...
    function buildFeatureToc(view) {
      ...
      function harvest(node) {
        if (node.tagName === "H3") {
          ...
          seq.push({ el: node, label: node.textContent.trim(), depth: 3 });
        }
      }
      scopes.forEach(function (scope) {
        if (scope.tagName === "H3") harvest(scope);
        $all("h3, h4, [data-toc-label]", scope).forEach(function (el) {
          if (el.tagName === "H3") {
            seq.push({ el: el, label: el.textContent.trim(), depth: 2 });
          } else if (el.tagName === "H4") {
            seq.push({ el: el, label: el.textContent.trim(), depth: 3 });
          } else {
            seq.push({ el: el, label: el.getAttribute("data-toc-label"), depth: 3 });
          }
        });
      });
      var html = "";
      ...
      // Fallback: if no h3s, use step titles so TOC is never empty
      if (!seq.length) {
        var fallbackHtml = "";
        var fallbackTargets = [];
        scopes.forEach(function (scope) {
          $all(".step", scope).forEach(function (el) {
            var t = el.querySelector(".step-title");
            if (t) fallbackTargets.push({ el: el, label: t.textContent.trim() });
          });
        });
        if (fallbackTargets.length) {
          fallbackTargets.forEach(function (item, i) {
            fallbackHtml += '<li><a class="toc-link depth-2" data-tref="f' + i + '" href="#">' + escapeHtml(item.label) + "</a></li>";
          });
          tocList.innerHTML = fallbackHtml;
          $all("a[data-tref]", tocList).forEach(function (a, i) {
            spyTargets.push({ el: fallbackTargets[i].el, link: a });
          });
        }
      }
    ...
    function restoreOverviewToc() {
      spyTargets = [];
      if (!tocList) return;
      tocList.innerHTML = overviewTocHTML;
      // Rebuild spyTargets from the static overview TOC links so scroll-spy works
      $all("a.toc-link", tocList).forEach(function (a) {
        var href = a.getAttribute("href") || "";
        var id = decodeURIComponent(href.replace(/^#/, ""));
        var el = id ? document.getElementById(id) : null;
        if (!el) return;
        spyTargets.push({ el: el, link: a });
    ...
    // Breadcrumb: separator SVG helper
    var SEP_SVG = '<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m9 6 6 6-6 6"/></svg>';

    function updateBreadcrumb(view) {
      var crumbs = $(".article-toolbar .crumbs");
      if (!crumbs) return;
      // Remove previous dynamic segments
      $all(".crumbs-dyn", crumbs).forEach(function (el) { el.remove(); });
      // Restore page-level crumb to plain span (not a link)
      var cur = crumbs.querySelector(".crumbs-current:not(.crumbs-dyn)");
      if (cur && cur.__origTag === "A") {
        var span = document.createElement("span");
        span.className = "crumbs-current";
        span.textContent = cur.textContent;
        cur.parentNode.replaceChild(span, cur);
        cur = span;
      }
      if (view !== "overview" && FEATURE_TITLE[view]) {
        // Demote page-level crumb to a clickable link back to overview
        if (cur && cur.tagName === "SPAN") {
          var a = document.createElement("a");
          a.__origTag = "A";
          a.className = "crumbs-current";
          a.href = "#";
          a.textContent = cur.textContent;
          a.addEventListener("click", function (e) { e.preventDefault(); setView("overview", true); });
          cur.parentNode.replaceChild(a, cur);
        }
        var sep = document.createElement("span");
        sep.className = "crumbs-sep crumbs-dyn";
        sep.innerHTML = SEP_SVG;
        var label = document.createElement("span");
        label.className = "crumbs-current crumbs-dyn";
        label.textContent = FEATURE_TITLE[view];
    ...
    var curPageName = (location.pathname.split("/").pop() || "");
    $all(".sb-sub, .submod-card, a[href^='#']").forEach(function (a) {
      if (a.classList.contains("toc-foot-link")) return;
      if (tocList && tocList.contains(a)) return;
      var href = a.getAttribute("href");
      if (!href || href === "#") return;
      // Normalize: strip own-page filename prefix
      var normalized = href;
      if (href.indexOf(curPageName + "#") === 0) normalized = href.slice(curPageName.length);
      if (normalized.charAt(0) !== "#") return;
      var id = decodeURIComponent(normalized.slice(1));
      if (!id) return;
      a.addEventListener("click", function (e) {
        e.preventDefault();
        if (isFeature[id]) {
    ...
    function revealActiveSub() {
      // sidebar state is preserved as-is; do not expand/collapse anything
    }
